...
import com.igobrilhante.github.models.{NotFoundException, UnexpectedErrorException}
  ...
  def apply(): ExceptionHandler =
    ExceptionHandler {
      case e: NotFoundException =>
        extractUri { uri =>
          complete(HttpResponse(NotFound, entity = HttpEntity(ContentTypes.`application/json`, e.asJson.dropNullValues.noSpaces)))
        }
